class Checkout:
    ...
    def calculateTotal(self):
        ...
        for item, cnt in self.items.items():
            total += self.canCulateTotalItemTotal(item, cnt)
        return total

    def canCulateTotalItemTotal(self, item, cnt):
        total = 0
        if item in self.discounts:
            discount = self.discounts[item]
            if cnt >= discount.nbrItems:
                total += self.canClcItemDiscTotal(item, cnt, discount)
            else:
    ...
    def canClcItemDiscTotal(self, item, cnt, discount):
        total = 0
